...
namespace Msml::Core {
    ...
    void HkPresent(Queue *pQueue, const QueuePresentDesc *pDesc) {
        ...
        if (!mIsInitialized) {
            ...
            HWND hwnd = graphicsDevice->hwnd;

            if (hwnd == nullptr) {
                pDesc->pSwapChain->mDx.pSwapChain->GetHwnd(&hwnd);
            }

            ImGui_ImplWin32_Init(hwnd);
            ogWndProc = reinterpret_cast<WNDPROC>(SetWindowLongPtr(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(WndProc)));
            ...
            pImguiQueue->mDx.pQueue->SetName(L"pImguiQueue");
            ...
            addCmdPool(pRenderer, &poolDesc, &pImGuiCmdPool);

            CmdDesc cmdDesc = {};
            cmdDesc.pPool = pImGuiCmdPool;
            addCmd(pRenderer, &cmdDesc, &pImGuiCmd);
            pImGuiCmd->mDx.pCmdList->SetName(L"pImGuiCmd");

            addFence(pRenderer, &pImGuiFence);
            pImGuiFence->mDx.pFence->SetName(L"pImGuiFence");
        ...
        resetCmdPool(pRenderer, pImGuiCmdPool);
        ...
        cmdBindRenderTargets(pImGuiCmd, &bindDesc);

        cmdSetViewport(pImGuiCmd, 0.0F, 0.0F, static_cast<float>(pRT->mWidth), static_cast<float>(pRT->mHeight), 0.0f, 1.0f);
        cmdSetScissor(pImGuiCmd, 0, 0, pRT->mWidth, pRT->mHeight);

        ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), pImGuiCmd->mDx.pCmdList);

        endCmd(pImGuiCmd);

        QueueSubmitDesc submitDesc = {};
        submitDesc.mCmdCount = 1;
        submitDesc.ppCmds = &pImGuiCmd;
        submitDesc.pSignalFence = pImGuiFence;

        queueSubmit(pQueue, &submitDesc);

        internal_waitForFences(1, &pImGuiFence);

        queuePresentHook.Original(pQueue, pDesc);

    }

    void TfWriteLogHooked(uint32_t level, const char *filename, int lineNumber, const char *message, ...) {
        ...
        auto local_level = System::Logger::LogLevel::kInfo;

        switch (level) {
            case 0:
            case 1:
                return;
            case 2: local_level = System::Logger::LogLevel::kDebug;
                break;
            case 4: local_level = System::Logger::LogLevel::kInfo;
                break;
            case 8: local_level = System::Logger::LogLevel::kWarning;
                break;
            case 16: local_level = System::Logger::LogLevel::kError;
                break;
            default: local_level = System::Logger::LogLevel::kInfo;
        }

        System::Logger::Log(local_level, false, filename, lineNumber,
                            "%s",
                            buffer.data());
    }

    void ImGuiHook::Install() {
        // tfWriteLogHook.Install(&TfWriteLogHooked);
