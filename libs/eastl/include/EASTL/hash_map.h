...
namespace eastl
{
	...
	///
	template <typename Key, typename T, typename Hash = eastl::hash<Key>, 
		typename Predicate = eastl::equal_to<Key>, typename Allocator = EASTLAllocatorType, bool bCacheHashCode = false>
	class hash_map
...
// Test stuff related with Spore
struct test_hashtable
{
	/* 00h */	int field_28;
	/* 04h */	void* mpBucketArray;  // node_type** mpBucketArray ?
	/* 08h */	int mnBucketCount;  // mnBucketCount
	/* 0Ch */	int mnElementCount;  // mnElementCount

	//prime_rehash_policy
	/* 10h */	float mfMaxLoadFactor;
	/* 14h */	float mfGrowthFactor;
	/* 18h */	int mnNextResize;

	/* 1Ch */	void* mAllocator;  // mAllocator // points to this+8 (in PFIndexModifiable)
};
// static_assert(sizeof(test_hashtable) == 0x20, "sizeof(test_hashtable) != 20h");
//
// static_assert(sizeof(eastl::hash_map<int, int>) == 0x20, "sizeof(hash_map) != 20h");
